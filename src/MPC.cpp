...
//bSet the reference velocity (max speed)
double ref_v = 100;  // 120 is OK!
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //1 Highest weight to reducing cte and epsi
    size_t t;
    for (t = 0; t < N; t++) {
      fg[0] += 1200*CppAD::pow(vars[t + cte_start], 2); 
      fg[0] += 1200*CppAD::pow(vars[t + epsi_start], 2);

      //To maintain velocity and does not stop.
      fg[0] += 10*CppAD::pow(vars[v_start + t] - ref_v, 2);
    }

    //2 Add initial a and delta values to the cost calculation
    for (t = 0; t < N - 1; t++) {
    ...
    //3 High weight to change in actuator inputs
